classdef ledConeTyping < nih.squirrellab.shared.protocols.SquirrelLabProtocol
     ...
     properties
        ...
        mLEDAmplitude = 5               % Pulse amplitude (V)
    ...
    methods
        ...
        function device = determineDevice(obj, epochNum)
            idx = mod(epochNum - 1, 3) + 1;
            if idx == 1
                device = obj.rig.getDevice('led591');
            elseif idx == 2
                device = obj.rig.getDevice('led530');
            else
                device = obj.rig.getDevice('led455');
            end
        end
